...
class Process:
    def __init__(self, file,opt, chunksize=10000):
        ...
        self.fields_dict_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),opt.fields_dict_path)
        if not os.path.exists(self.fields_dict_path):
            self.one_hot_express()
        else:
            print('loading data...')
            self.fields_dict = np.load(self.fields_dict_path).item()
            
            self.feature2field = self.fields_dict['feature2field'] 
            self.feature_length = len(self.feature2field.keys())
        self.data_length = self.fields_dict['all_count']
    ...
    def one_hot_express(self):
        ...
        device_ip = dict()
        all_index = 0
        for kk, data in enumerate(copy.copy(self.data)):
            # if kk > 100:
            #     break
        ...
        ind = 0
        self.fields_dict = {}
        self.feature2field = {} #哪些特征属于同一域，比如hour的特征ind为[1,2,...,24]都属于一个域
        field_index = 0
        for kk,field in enumerate(direct_encoding_fields):
            ...
            for value in list(field_sets):
                ...
                self.feature2field[ind] = field_index
                ind += 1
            field_index += 1
        ...
        for kk,field in enumerate(frequency_encoding_fields):
            ...
            for k,count in field2count.items():
                if count < 10:
                    if index_rare == None:
                        ...
                        self.feature2field[index_rare] = field_index
                else:
                    field_dict[k] = ind
                    self.feature2field[ind] = field_index
                    ind += 1
            ...
            print('\rhave process : %06d'%kk,end='',flush=True)
        print()
        self.fields_dict['all_count'] = all_index
        self.fields_dict['feature2field'] = self.feature2field
        # one-hot 长度
        np.save(self.fields_dict_path,self.fields_dict)
        self.feature_length = len(self.feature2field.keys())
    ...
    def get_data(self):
        ...
        idx = next_element[-1]
        label.set_shape(self.opt.batch_size)
        
        return inputs,label,idx

    
    def _sample_data(self):
        self.data = pd.read_csv(self.file,chunksize=self.chunksize)
        for kk,data in enumerate(copy.copy(self.data)):
            # if kk > 10:
            #     break
    ...
    def _one_hot_representation(self,sample):
        ...
        for field in self.fields_dict.keys():
            ...
            if field != 'all_count' and field != 'feature2field':
                if field == 'hour':
